...
function VendorPaymentModal({ vendor, onClose, onSuccess, theme }) {
	...
	const fetchPendingInvoices = useCallback(async () => {
		...
		try {
			const token = localStorage.getItem('accessToken');
			const res = await axios.get(
				`/api/vendor-summary/${vendor.vendor_id}/pending-invoices`,
				{ headers: { Authorization: `Bearer ${token}` } }
			);
	...
	const handleInvoiceSelect = (opt) => {
		const selected = invoices.find(inv => String(inv.id) === String(opt?.value));
		setInvoiceId(opt?.value || '');
		setInvoiceType(selected?.type || '');
		setAmount(selected ? parseFloat(selected.debit_entry).toFixed(2) : '');
	};
	...
	const handleConfirmPayment = async () => {
		...
		try {
			...
			const res = await axios.post(
				...
				{ invoice_id: invoiceId, invoice_type: invoiceType, payment_amount: amount },
				{ headers: { Authorization: `Bearer ${token}` } }
			);
			if (res.data.success) {
							...
							<ThemeUI.FormField label="Select Invoice" required>
								<ThemeUI.Select
									...
									options={invoices.map(inv => ({
										value: String(inv.id),
										label: `${inv.typelabel} · ${inv.invoice_number || `Invoice #${inv.id}`} · ${inv.site?.name || ''} · Due: ${fmt(inv.debit_entry)}`
									}))}
