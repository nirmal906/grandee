...
function LabourEntry() {
    ...
    const getUserPermissions = useCallback(() => {
        try {
            ...
            return perms.labourentry || { can_add: false, can_edit: false, can_delete: false, can_view: false };
        } catch { return { can_add: false, can_edit: false, can_delete: false, can_view: false }; }
    }, []);
    ...
    // ── open modal from location state ──
    useEffect(() => {
        if (location.state?.openModal && location.state?.siteId) {
            setEditingEntry({ isNew: true });
            setFormData({ ...emptyForm, site_id: location.state.siteId });
            setBackendErrors({});
            setIsModalOpen(true);
            window.history.replaceState({}, document.title);
        }
    }, [location.state, emptyForm]);

    // ── fetch helpers ──
    const fetchActiveLabours = useCallback(async () => {
        try {
            const res = await axios.get('/api/labour-entry/active-labours');
            if (res.data.success) setActiveLabours(res.data.data);
        } catch (err) { console.error('Failed to fetch active labours:', err); }
    }, []);

    const fetchActiveVendors = useCallback(async () => {
        try {
            const res = await axios.get('/api/material-entry/active-vendors');
    ...
    const fetchActiveSites = useCallback(async () => {
        try {
            ...
            if (res.data.success) setActiveSites(res.data.data);
        } catch (err) { console.error('Failed to fetch active sites:', err); }
    }, []);
    ...
    const fetchLabourEntries = useCallback(async () => {
        ...
        try {
            ...
            // Non-admin: no approval filter needed (backend already scopes to their entries)
            // Admin: use the approvalFilter dropdown (default: approved in main table)
            if (isAdmin && approvalFilter) params.approval_status = approvalFilter;

            const response = await axios.get('/api/labour-entry', { params });
    ...
    }, [currentPage, perPage, searchQuery, statusFilter, labourFilter, siteFilter, dateFrom, dateTo, vendorFilter, approvalFilter, isAdmin]);

    useEffect(() => { fetchLabourEntries(); }, [fetchLabourEntries]);
    useEffect(() => { fetchActiveLabours(); fetchActiveSites(); fetchActiveVendors(); }, []);
    ...
    useEffect(() => {
        ...
        const t = setTimeout(() => {
            ...
            if (!isDeleting && currentCharIndex < word.length) {
                setPlaceholder(`Search by ${word.substring(0, currentCharIndex + 1)}...`);
                setCurrentCharIndex(p => p + 1);
            } else if (isDeleting && currentCharIndex > 0) {
                setPlaceholder(`Search by ${word.substring(0, currentCharIndex - 1)}...`);
                setCurrentCharIndex(p => p - 1);
            } else if (!isDeleting && currentCharIndex === word.length) {
                setIsPaused(true); setIsDeleting(true);
            } else if (isDeleting && currentCharIndex === 0) {
                setIsDeleting(false);
    ...
    const totalAmount = useMemo(() => {
        const w = Number(formData.no_of_workers)  || 0;
        const r = Number(formData.rate_per_worker) || 0;
        return w * r;
    }, [formData.no_of_workers, formData.rate_per_worker]);

    // ── auto debit/credit ──
    useEffect(() => {
        if (isUpdatingRef.current || !isAdmin) return;
        if (totalAmount === 0) {
        ...
        if (formData.credit_entry !== '' && credit >= 0 && credit <= totalAmount) {
            const calc = totalAmount - credit;
            if (Math.abs(calc - debit) > 0.01) setFormData(prev => ({ ...prev, debit_entry: calc.toFixed(2) }));
        } else if (formData.debit_entry !== '' && debit >= 0 && debit <= totalAmount) {
            const calc = totalAmount - debit;
            if (Math.abs(calc - credit) > 0.01) setFormData(prev => ({ ...prev, credit_entry: calc.toFixed(2) }));
        }
    ...
    const handleAddClick = () => {
        if (!permissions.can_add) { toast.error("You don't have permission to add labour entry"); return; }
        setEditingEntry({ isNew: true });
    ...
    const handleEditClick = (entry) => {
        if (!permissions.can_edit) { toast.error("You don't have permission to edit labour entry"); return; }
        // Non-admin: only pending
        if (!isAdmin && entry.approval_status !== 'pending') {
            toast.error('Only pending entries can be edited'); return;
        }
    ...
    const handleSave = async () => {
        setIsLoading(true);
        try {
            const url    = editingEntry?.isNew ? '/api/labour-entry' : `/api/labour-entry/${editingEntry.id}`;
            const method = editingEntry?.isNew ? 'post' : 'put';

            // Non-admin: only send limited fields
            const payload = isAdmin ? formData : {
                site_id:       formData.site_id,
                labour_id:     formData.labour_id,
                vendor_id:     formData.vendor_id,
                date:          formData.date,
                no_of_workers: formData.no_of_workers,
            };
    ...
    const handleDelete = async (id, approvalStatus) => {
        if (!isAdmin && approvalStatus !== 'pending') { toast.error('Only pending entries can be deleted'); return; }
    ...
    const handleLabourChange = (selectedOption) => {
        ...
        if (labourId) {
            const selected = activeLabours.find(l => l.id === labourId);
            setFormData(prev => ({
                ...prev,
                labour_id:       labourId,
                rate_per_worker: (isAdmin && selected?.standard_rate) ? selected.standard_rate : prev.rate_per_worker,
                debit_entry:     '',
                credit_entry:    '',
            }));
        } else {
            setFormData(prev => ({ ...prev, labour_id: '', rate_per_worker: '', debit_entry: '', credit_entry: '' }));
        }
    };

    const handleRateChange = (e) => {
        setFormData(prev => ({ ...prev, rate_per_worker: e.target.value, debit_entry: '', credit_entry: '' }));
    ...
    const handleCreditChange = (e) => {
        ...
        if (v === '' || (!isNaN(v) && Number(v) >= 0)) {
            setFormData(prev => ({ ...prev, credit_entry: v, debit_entry: '' }));
    ...
    const handleDebitChange = (e) => {
        const v = e.target.value;
        if (v === '' || (!isNaN(v) && Number(v) >= 0)) {
            setFormData(prev => ({ ...prev, debit_entry: v, credit_entry: '' }));
    ...
    const columnDefs = useMemo(() => {
        const base = [
            ...
            { headerName: 'Site',   field: 'site.name',   flex: 1, minWidth: 160 },
            {
                headerName: 'Date', field: 'date', width: 120,
                valueFormatter: p => p.value ? new Date(p.value).toLocaleDateString() : '—',
            },
            {
                headerName: 'Vendor', minWidth: 120, flex: 1,
            ...
            {
                headerName: 'Rate/Worker', width: 130,
                valueGetter: p => p.data.rate_per_worker,
                valueFormatter: p => p.value != null ? `₹${Number(p.value).toFixed(2)}` : '—',
            },
            {
                headerName: 'Total', width: 120,
                valueGetter: p => {
                    const w = p.data?.no_of_workers   || 0;
                    const r = p.data?.rate_per_worker || 0;
                    return (w && r) ? w * r : null;
                },
                valueFormatter: p => p.value != null ? `₹${Number(p.value).toFixed(2)}` : '—',
            },
            {
                headerName: 'Paid', field: 'credit_entry', width: 110,
                valueFormatter: p => p.value != null ? `₹${Number(p.value).toFixed(2)}` : '—',
            },
            {
                headerName: 'Pending', field: 'debit_entry', width: 110,
                valueFormatter: p => p.value != null ? `₹${Number(p.value).toFixed(2)}` : '—',
            },
            {
            ...
            {
                headerName: 'Approval', field: 'approval_status', width: 110,
                cellRenderer: p => (
            ...
            {
                headerName: 'Rejection Reason', field: 'rejection_reason', flex: 1, minWidth: 160,
                valueGetter: p => p.data.rejection_reason || '—',
            },
            { headerName: 'Created By', width: 130, valueGetter: p => p.data?.creator?.name || '—' },
            { headerName: 'Updated By', width: 130, valueGetter: p => p.data?.updater?.name || '—' },
        ];
        ...
        if (permissions.can_edit || permissions.can_delete) {
            base.push({
                ...
                cellRenderer: p => {
                    const canEdit   = isAdmin || p.data.approval_status === 'pending';
                    const canDelete = isAdmin || p.data.approval_status === 'pending';
                    return (
                        <div className="flex items-center gap-2">
                            ...
                            {permissions.can_delete && canDelete && (
                                <button onClick={() => handleDelete(p.data.id, p.data.approval_status)} className="p-1 text-red-600 hover:text-red-800" title="Delete">
                                    <Trash2 size={16} />
                                </button>
                            )}
                        </div>
        ...
        return base;
    }, [currentPage, perPage, permissions, isAdmin]);

    // ── form render ──
    const renderForm = () => (
        <div className="space-y-6">
            <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-4">
                {/* ── Fields shown to ALL roles ── */}
                ...
                <ThemeUI.FormField label="Vendor" name="vendor_id" error={backendErrors.vendor_id}>
                    <ThemeUI.Select
                        ...
                        onChange={opt => {
                            setFormData(prev => ({ ...prev, vendor_id: opt?.value || '' }));
                ...
                <ThemeUI.FormField label="Labour" name="labour_id" error={backendErrors.labour_id} required>
                    <ThemeUI.Select
                        ...
                        options={activeLabours.map(l => ({
                            value: l.id,
                            label: isAdmin
                                ? `${l.name} (₹${Number(l.standard_rate).toFixed(2)})`
                                : l.name,
                        }))}
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-1.5">
                                ...
                                <ThemeUI.Checkbox
                                    ...
                                    onChange={e => {
                                        if (e.target.checked) {
                                            setFormData(prev => ({ ...prev, credit_entry: totalAmount.toFixed(2), debit_entry: '0.00' }));
                                        } else {
                                            setFormData(prev => ({ ...prev, credit_entry: '', debit_entry: '' }));
                                        }
                                    }}
                            ...
                            {backendErrors.credit_entry && (
                                <p className="mt-1 text-xs text-red-500">{backendErrors.credit_entry}</p>
                            )}
                        </div>
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-1.5">
                                ...
                                <ThemeUI.Checkbox
                                    ...
                                    onChange={e => {
                                        if (e.target.checked) {
                                            setFormData(prev => ({ ...prev, debit_entry: totalAmount.toFixed(2), credit_entry: '0.00' }));
                                        } else {
                                            setFormData(prev => ({ ...prev, debit_entry: '', credit_entry: '' }));
                                        }
                                    }}
                            ...
                            {backendErrors.debit_entry && (
                                <p className="mt-1 text-xs text-red-500">{backendErrors.debit_entry}</p>
                            )}
                        </div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="font-semibold text-green-700">₹{Number(formData.credit_entry || 0).toFixed(2)}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <span className="text-slate-500">Due:</span>
                                <span className="font-semibold text-red-700">₹{Number(formData.debit_entry || 0).toFixed(2)}</span>
                            </div>
                        </div>
                        {Math.abs((Number(formData.debit_entry || 0) + Number(formData.credit_entry || 0)) - totalAmount) > 0.01 && (
            ...
            {!isAdmin && (
                <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700">
                    ℹ️ Your entry will be submitted for Admin approval. Rate and payment details will be set by the Admin.
                </div>
            )}
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
                ...
                >
                    {isLoading ? (
                        ...
                        isAdmin ? 'Create Entry' : 'Submit for Approval'
                    ) : (
                        'Update Entry'
                    )}
                </ThemeUI.Button>
        ...
        <Layout>
            <div className="flex items-center mb-4">
                ...
                <nav className="flex items-center text-sm text-gray-500 whitespace-nowrap overflow-x-auto">
                    <ol className="flex items-center">
                        ...
                        <li style={{ color: theme.primaryGradientStart }} className="font-medium">Labour Entry</li>
                    </ol>
                </nav>
            </div>
            ...
            >
                <div className="space-y-4">
                    ...
                    <ThemeUI.FormField label="Vendor">
                        <ThemeUI.Select
                            value={vendorFilter}
                    ...
                    {isAdmin && (
                        <ThemeUI.FormField label="Approval Status">
                            <ThemeUI.Select
                                ...
                                onChange={opt => { setApprovalFilter(opt?.value || ''); setCurrentPage(1); }}
                                options={[
                    ...
                    <div className="flex gap-2 pt-4">
                        <ThemeUI.Button
                            onClick={() => {
                                setSiteFilter(''); setLabourFilter(''); setVendorFilter('');
                                setDateFrom(''); setDateTo(''); setStatusFilter(''); setApprovalFilter('');
                                setCurrentPage(1); setIsFilterOffcanvasOpen(false);
                            }}
                            gradientColors={{ start: theme.secondaryGradientStart, end: theme.secondaryGradientEnd }}
                            className="flex-1"
                        >
